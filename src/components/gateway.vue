  ...
  <div>
    ...
    <div class="index-filter-container">
      <el-form :model="searchInfo" :inline="true" ref="searchInfo" label-position="left" label-width="0px" class="demo-ruleForm card-box loginform">
        ...
        <el-form-item prop="simCardId">
          <el-select v-model="searchInfo.simCardId"  placeholder="SIM卡" clearable>
            <el-option
              v-for="item in simIdList"
              :key="item.id"
        ...
        <el-form-item prop="parkingId">
          <el-select v-model="searchInfo.parkingId"  placeholder="停车场" clearable>
            <el-option
        ...
        <el-form-item>
          <el-button class="filter-item" style="margin-left: 10px;" @click="_clickFlag('add', '新增网关')" type="primary" icon="edit">添加</el-button>
        </el-form-item>
      </el-form>
    </div>
    <!-------------------------------------列表-栏目------------------------>
    <div class="index-main-table">
      <el-table :data="list"  border v-loading="listLoading" element-loading-text="正在加载中" fit highlight-current-row  style="width: 100%" >
        ...
        <el-table-column prop="phoneNumber" label="SIM卡" min-width="15%" align="center"></el-table-column>
        <el-table-column prop="parkingName" label="所属停车场" min-width="15%" align="center"></el-table-column>
        <el-table-column prop="deviceStatus" label="设备状态" min-width="10%" align="center"></el-table-column>
        <el-table-column prop="remark" label="描述" min-width="15%" align="center" :show-overflow-tooltip="true"></el-table-column>
        <el-table-column prop="createTime" label="创建时间" min-width="20%" align="center"></el-table-column>
        <!------------------------------列表-操作栏------------------------------->
        <el-table-column align="center" label="操作" min-width="10%" header-align="center">
          <template slot-scope="scope">
            <el-button size="small" type="info" @click="_clickFlag('edit', '', '编辑网关', scope.row)">编辑</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    <el-dialog :title="dialogTitle" size="tiny" :visible.sync="dialogShow" align="left" @close="dialogInfo={}">
      <el-form ref="dialogInfo" :model="dialogInfo" :rules="rules">
        ...
        <el-form-item label="所属SIM卡" :label-width="formLabelWidth" prop="simCardId">
        ...
        <el-form-item label="所属停车场" :label-width="formLabelWidth" prop="parkingId">
          <el-select v-model="dialogInfo.parkingId"  placeholder="请选择" style="width: 80%">
            <el-option
              v-for="item in parkingIdList"
              :key="item.id"
              :label="item.name"
...
<script type="text/javascript">
  ...
  export default {
    ...
    methods: {
      ...
      _getParkingId() {
        axios.post(baseUrl + '/parking/list').then((res) => {
          if (res.data.code == 200) {
            this.parkingIdList = res.data.data
          }
        }).catch((err) => {
          console.log(err)
        })
